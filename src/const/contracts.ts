export const MARKETPLACE_ADDRESS: string =
  "0x586d90eceDAf6627832f1B6081CAfc4Ea27fAf6A";

export const NFT_COLLECTIONS: string[] = [
  "0x0924319a7524cf023356Ace4D5018fADDE0c60C8", // Bad_Azz_Bumba_Beez
  "0x317F0FCB1d14C8aaA33F839B43B1aa92845a8145", // Betaverse_Bumba_Beez
  "0xA3DaEd128c483e38984f8374916A441a22CD8aDd", // Core_Bumba_Beez
];

// (Optional) If you want to look up by name elsewhere:
export const NFT_COLLECTIONS_MAP: Record<string, string> = {
  Bad_Azz_Bumba_Beez:
    "0x0924319a7524cf023356Ace4D5018fADDE0c60C8",
  Betaverse_Bumba_Beez:
    "0x317F0FCB1d14C8aaA33F839B43B1aa92845a8145",
  Core_Bumba_Beez:
    "0xA3DaEd128c483e38984f8374916A441a22CD8aDd",
};

export const NFT_COLLECTION = NFT_COLLECTIONS[0];

export const STAKING_CONTRACTS: Record<string, string> = {
  Queen_Bee_Nectar:
    "0xD86965fE1436B01dBD933cE433359D2255F2135D",
  Degen_Bee_Nectar:
    "0x6b391d65f21CA93A39E3B9715C55b320f580aD1a",
  Beta_Bee_Nectar:
    "0xB04fcfe7EB075BaC7040f1f647c12A55FA4FbB0f",
};

export const BRAP_TOKEN_ADDRESS: string =
  "0x5b3Ff4d494E9Ee69eE0f52Ab9656cFfe99D4839E";

export const NETWORK: number = 43114;
